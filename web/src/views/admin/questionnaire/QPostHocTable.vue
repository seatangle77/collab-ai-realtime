...
import type { QPostHocResult } from '../../../api/admin/questionnaire-analysis'
import { conditionLabel, formatNumber, pValueText } from './reportHelpers'

defineProps<{
  loading: boolean
  postHocTests: QPostHocResult[]
}>()

function methodLabel(method: QPostHocResult['method']): string {
  if (method === 'tukey_hsd') return 'Tukey HSD'
  if (method === 'dunn_bonferroni') return 'Dunn + Bonferroni'
  return '—'
}

function statusLabel(status: QPostHocResult['status']): string {
  const labels: Record<QPostHocResult['status'], string> = {
    ok: '已计算',
    not_applicable: '不适用',
  ...
  <el-card class="analysis-card" shadow="never">
    <template #header>
      <div class="card-title">
        ...
        <span>仅三条件且全局检验 p &lt; 0.05 时执行</span>
      </div>
    </template>
