...
import type { EnaPostHocResult } from '../../../api/admin/ena-analysis'
import { conditionLabel, formatNumber, pValueText, postHocMethodLabel, postHocStatusLabel } from './reportHelpers'

defineProps<{
  loading: boolean
  postHocTests: EnaPostHocResult[]
}>()
</script>
...
<template>
  <el-card class="analysis-card" shadow="never">
    ...
    <div v-for="item in postHocTests" :key="item.metric" class="post-hoc-section">
      <div class="section-header">
        <span class="metric-label">{{ item.label }}</span>
        <el-tag v-if="item.status === 'ok'" size="small" type="success">{{ postHocMethodLabel(item.method) }}</el-tag>
        <el-tag v-else size="small" type="info">{{ postHocStatusLabel(item.status) }}</el-tag>
        <span class="section-note">{{ item.note }}</span>
      </div>
      <el-table
        v-if="item.status === 'ok' && item.pairs.length > 0"
        :data="item.pairs"
        border
        class="compact-table pair-table"
      >
...
.post-hoc-section { padding: 12px 14px; border-bottom: 1px solid #f0f4f9; }
.post-hoc-section:last-child { border-bottom: 0; }
.section-header { display: flex; align-items: center; gap: 8px; margin-bottom: 10px; }
.metric-label { color: #1f2a3d; font-size: 14px; font-weight: 700; }
.section-note { color: #748197; font-size: 12px; }
