...
<h2>4. ${zh ? '产生率可视化' : 'Rate visualizations'}</h2><div class="figure"><div class="boxplot-grid">${rateFigures}</div><div class="caption">${zh ? '图1　三种实验条件下CoI观点产生率的会话级分布。箱体表示中位数和四分位区间，须线为1.5倍四分位距范围，小型点符号为每场会话；圆形、方形和三角形分别对应无辅助、智能眼镜和APP通知，菱形与误差线表示均值及95%置信区间。' : 'Figure 1. Session-level CoI rate distributions across the three conditions. Boxes show medians and interquartile ranges, whiskers extend to 1.5 IQR, small circle, square, and triangle symbols identify sessions in the three conditions, and diamonds with error bars show means and 95% confidence intervals.'}</div></div>
