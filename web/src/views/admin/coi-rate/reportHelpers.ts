...
export function buildCoiRateReportHtml(
  ...
  const panels = [
    { key: 'total_rate', title: 'All Four CoI Phases', subtitle: 'TE + EX + IN + RE codes per session minute', wide: true },
    { key: 'te_rate', title: 'TE · Triggering Event', subtitle: 'Codes per minute', wide: false },
    { key: 'ex_rate', title: 'EX · Exploration', subtitle: 'Codes per minute', wide: false },
    { key: 'in_rate', title: 'IN · Integration', subtitle: 'Codes per minute', wide: false },
    { key: 're_rate', title: 'RE · Resolution', subtitle: 'Codes per minute', wide: false },
  ] as const
  ...
  const meanTotalRows = meanPanels[0]!.values.map((item, index) => `<text x="205" y="${76 + index * 48}" text-anchor="end" class="condition">${escapeHtml(EN_CONDITIONS[item.condition] ?? item.condition)}</text><rect x="225" y="${57 + index * 48}" width="800" height="24" fill="#eef2f7"/><rect x="225" y="${57 + index * 48}" width="${Math.max(0, Math.min(800, item.value / totalMeanMaximum * 800))}" height="24" fill="${barColors[item.condition] ?? '#374151'}"/><text x="1045" y="${76 + index * 48}" class="value">${number(item.value)}</text>`).join('')
  const meanPhasePanels = meanPanels.slice(1).map((panel, panelIndex) => {
    const left = 30 + (panelIndex % 2) * 590
    const top = 245 + Math.floor(panelIndex / 2) * 225
    const rows = panel.values.map((item, index) => `<text x="${left + 145}" y="${top + 44 + index * 46}" text-anchor="end" class="condition">${escapeHtml(EN_CONDITIONS[item.condition] ?? item.condition)}</text><rect x="${left + 165}" y="${top + 26 + index * 46}" width="310" height="23" fill="#eef2f7"/><rect x="${left + 165}" y="${top + 26 + index * 46}" width="${Math.max(0, Math.min(310, item.value / phaseMeanMaximum * 310))}" height="23" fill="${barColors[item.condition] ?? '#374151'}"/><text x="${left + 490}" y="${top + 44 + index * 46}" class="value">${number(item.value)}</text>`).join('')
    return `<text x="${left}" y="${top}" class="chart-title">${escapeHtml(panel.title.replace(' · ', ' '))}</text><text x="${left + 540}" y="${top}" text-anchor="end" class="axis-note">Codes per minute</text>${rows}`
  }).join('')
  const meanRateSvg = `<svg class="mean-rate-svg" viewBox="0 0 1200 700" role="img" aria-label="Mean CoI idea-generation rates by condition"><defs><style>text{font-family:Arial,Helvetica,sans-serif;text-rendering:geometricPrecision}.chart-title{fill:#0f172a;font-size:20px;font-weight:800}.condition{fill:#1e293b;font-size:15px;font-weight:700}.value{fill:#1e293b;font-size:15px;font-weight:750}.axis-note{fill:#526071;font-size:14px;font-weight:600}</style></defs><text x="30" y="34" class="chart-title">All Four CoI Phases</text><text x="1170" y="34" text-anchor="end" class="axis-note">Codes per minute</text>${meanTotalRows}<line x1="30" x2="1170" y1="220" y2="220" stroke="#dbe3ee" stroke-width="1.5"/>${meanPhasePanels}</svg>`
  ...
  `).join('')
  const forestItems = report.contrasts.filter(item => item.metric !== 'total_rate')
  ...
  const forestHeight = 90 + forestItems.length * 64
  const forestRows = forestItems.map((item, index) => {
    const y = 78 + index * 64
    const color = barColors[item.comparison_condition] ?? '#374151'
    ...
    const interval = item.ci_low != null && item.ci_high != null
      ...
      : ''
    return `<line x1="25" x2="1175" y1="${50 + index * 64}" y2="${50 + index * 64}" stroke="#e2e8f0"/><text x="350" y="${y - 5}" text-anchor="end" class="metric">${escapeHtml(metric)}</text><text x="350" y="${y + 13}" text-anchor="end" class="comparison">${escapeHtml(EN_CONDITIONS[item.comparison_condition] ?? item.comparison_condition)} − No Assistance</text>${interval}<rect x="${contrastX(item.mean_difference) - 6}" y="${y - 6}" width="12" height="12" fill="${color}"/><text x="925" y="${y + 5}" class="effect">${item.mean_difference > 0 ? '+' : ''}${number(item.mean_difference, 2)} [${number(item.ci_low, 2)}, ${number(item.ci_high, 2)}]</text>`
  }).join('')
  const forestSvg = `<svg viewBox="0 0 1200 ${forestHeight}" role="img" aria-label="Rate differences relative to No Assistance"><defs><style>text{font-family:Arial,Helvetica,sans-serif;text-rendering:geometricPrecision}.metric{fill:#0f172a;font-size:15px;font-weight:750}.comparison{fill:#526071;font-size:13px;font-weight:600}.effect{fill:#1e293b;font-size:14px;font-weight:700}.tick{fill:#526071;font-size:13px;font-weight:650}</style></defs><line x1="640" x2="640" y1="42" y2="${forestHeight - 24}" stroke="#64748b" stroke-width="2"/><text x="390" y="25" text-anchor="middle" class="tick">−${contrastMaximum.toFixed(1)}</text><text x="640" y="25" text-anchor="middle" class="tick">0</text><text x="890" y="25" text-anchor="middle" class="tick">+${contrastMaximum.toFixed(1)}</text>${forestRows}</svg>`
