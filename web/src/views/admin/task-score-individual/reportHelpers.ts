...
function pairedScoreSvg(report: TaskScoreIndividualAnalysisResult): string {
  ...
  const lines = report.conditions.flatMap((condition, conditionIndex) => {
    ...
    return rows.map((item, index) => {
      ...
      return `<g><line x1="${x1}" y1="${y(item.individual_score)}" x2="${x2}" y2="${y(item.group_score)}" stroke="${color}" stroke-width="1.8" stroke-opacity=".5"/><circle cx="${x1}" cy="${y(item.individual_score)}" r="4" fill="${color}"><title>${esc(item.participant_id)}: IS ${item.individual_score}</title></circle><circle cx="${x2}" cy="${y(item.group_score)}" r="4" fill="${color}"><title>${esc(item.group_id)}: GS ${item.group_score}; improvement ${item.improvement}</title></circle></g>`
    })
  }).join('')
  return `<svg class="paired-svg" viewBox="0 0 ${width} ${height}" role="img" aria-label="Paired individual and group final scores">${grid}<line x1="${margin.left}" x2="${margin.left}" y1="${margin.top}" y2="${height - margin.bottom}" class="axis"/>${panels}${lines}<text x="22" y="250" transform="rotate(-90 22 250)" text-anchor="middle" class="axis-label">Task Score (lower = better)</text><g class="legend"><text x="350" y="488" fill="#16a34a">● Group better</text><text x="535" y="488" fill="#dc2626">● Group worse</text><text x="720" y="488" fill="#64748b">● Unchanged</text></g></svg>`
}
...
body{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;color:#172033;margin:32px;line-height:1.55}h1{margin-bottom:4px}h2{margin-top:28px;padding-bottom:6px;border-bottom:1px solid #dbe3ee}p.note{color:#526071;background:#f8fafc;padding:12px 14px;border-left:4px solid #2563eb}table{width:100%;border-collapse:collapse;margin:12px 0}th,td{border:1px solid #dbe3ee;padding:8px 10px;text-align:left;vertical-align:top}th{background:#f4f7fb}.ok{color:#047857}.warning{color:#b45309}.paired-svg .grid-line{stroke:#e2e8f0}.paired-svg .axis{stroke:#94a3b8}.paired-svg .divider{stroke:#cbd5e1;stroke-dasharray:4 5}.paired-svg text{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}.paired-svg .tick-label,.paired-svg .axis-label,.paired-svg .phase-label{fill:#64748b;font-size:13px}.paired-svg .condition-label{fill:#1e293b;font-size:16px;font-weight:700}.paired-svg .legend text{font-size:13px;font-weight:600}${INTERACTIVE_CHART_CSS}@media print{body{margin:12mm}}
