...
function pairedScoreSvg(report: TaskScoreIndividualAnalysisResult): string {
  const width = 1200
  const height = 560
  const margin = { top: 92, right: 34, bottom: 112, left: 86 }
  ...
  const panels = report.conditions.map((condition, index) => {
    ...
    const summary = report.improvement_summaries.find(item => item.condition === condition)
    const test = report.within_condition_tests.find(item => item.condition === condition)
    return `${divider}<text x="${panelLeft + panelWidth * .5}" y="54" text-anchor="middle" class="condition-label">${esc(conditionLabelEn(condition))}</text><text x="${panelLeft + panelWidth * .5}" y="75" text-anchor="middle" class="panel-stat">n=${summary?.individual_count ?? 0} individuals / ${summary?.group_count ?? 0} groups · MΔ=${academicNumber(summary?.mean, 2)} · Holm ${academicPValue(test?.p_value_adjusted)}</text><text x="${panelLeft + panelWidth * .28}" y="490" text-anchor="middle" class="phase-label">Individual IS</text><text x="${panelLeft + panelWidth * .72}" y="490" text-anchor="middle" class="phase-label">Group GS</text>`
  }).join('')
  const lines = report.conditions.flatMap((condition, conditionIndex) => {
    ...
    return rows.map((item, index) => {
      ...
      const color = item.improvement > 0 ? '#0072B2' : item.improvement < 0 ? '#D55E00' : '#6B7280'
  ...
  const omnibus = `Omnibus permutation ${academicPValue(report.statistical_test.p_value)}${report.statistical_test.effect_size_name && report.statistical_test.effect_size != null ? ` · ${report.statistical_test.effect_size_name} = ${academicNumber(report.statistical_test.effect_size, 2)}` : ''}`
  return `<svg class="paired-svg" viewBox="0 0 ${width} ${height}" role="img" aria-label="Paired individual and group final scores"><defs><style>text{font-family:Arial,Helvetica,sans-serif;text-rendering:geometricPrecision}.grid-line{stroke:#d4dde9;stroke-width:1.25}.axis{stroke:#64748b;stroke-width:1.8}.divider{stroke:#94a3b8;stroke-width:1.25;stroke-dasharray:4 5}.tick-label{fill:#334155;font-size:15px;font-weight:650}.axis-label{fill:#1e293b;font-size:16px;font-weight:700}.phase-label{fill:#334155;font-size:15px;font-weight:700}.condition-label{fill:#0f172a;font-size:19px;font-weight:800}.figure-title{fill:#0f172a;font-size:19px;font-weight:800}.statistic-label{fill:#334155;font-size:12px;font-weight:700}.panel-stat{fill:#526071;font-size:11px;font-weight:650}.legend text{font-size:14px;font-weight:700}</style></defs><text x="24" y="28" class="figure-title">Individual Score → Group Final Score</text><text x="${width - 24}" y="28" text-anchor="end" class="statistic-label">${esc(omnibus)}</text>${grid}<line x1="${margin.left}" x2="${margin.left}" y1="${margin.top}" y2="${height - margin.bottom}" class="axis"/><line x1="${margin.left}" x2="${width - margin.right}" y1="${height - margin.bottom}" y2="${height - margin.bottom}" class="axis"/>${panels}${lines}<text x="22" y="270" transform="rotate(-90 22 270)" text-anchor="middle" class="axis-label">Task Score (lower = better)</text><g class="legend"><text x="350" y="520" fill="#0072B2">● Group better</text><text x="535" y="520" fill="#D55E00">● Group worse</text><text x="720" y="520" fill="#6B7280">● Unchanged</text></g><text x="600" y="550" text-anchor="middle" class="axis-label">Measurement Stage within Experimental Condition</text></svg>`
}
