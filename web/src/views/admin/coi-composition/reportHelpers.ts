...
function figuresHtml(
  ...
  const stackedChart = `<section class="stacked-overview"><h3>${language === 'zh' ? '条件平均构成' : 'Mean composition by condition'}<span>${language === 'zh' ? '每个条形合计为100%；条形内直接标注阶段与平均占比' : 'Each bar totals 100%; phases and mean proportions are labeled directly'}</span></h3><svg viewBox="0 0 760 182" role="img"><defs><pattern id="report-phase-te" width="8" height="8" patternUnits="userSpaceOnUse"><rect width="8" height="8" fill="#374151"/></pattern><pattern id="report-phase-ex" width="8" height="8" patternUnits="userSpaceOnUse"><rect width="8" height="8" fill="#2563eb"/></pattern><pattern id="report-phase-in" width="8" height="8" patternUnits="userSpaceOnUse"><rect width="8" height="8" fill="#16a34a"/></pattern><pattern id="report-phase-re" width="8" height="8" patternUnits="userSpaceOnUse"><rect width="8" height="8" fill="#f97316"/></pattern></defs>${stackedRows}</svg></section>`
