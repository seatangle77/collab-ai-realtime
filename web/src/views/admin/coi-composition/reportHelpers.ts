...
function figuresHtml(
  ...
  const stackedChart = `<section class="stacked-overview"><h3>Mean Composition by Condition<span>Each bar totals 100%; phases and mean proportions are labeled directly.</span></h3><svg viewBox="0 0 800 282" role="img"><defs><style>text{font-family:Arial,Helvetica,sans-serif;text-rendering:geometricPrecision}.stack-label{fill:#0f172a;font-size:15px;font-weight:750}.stack-text{fill:#fff;font-size:13px;font-weight:750;paint-order:stroke;stroke:#17203388;stroke-width:2px}.overview-title{fill:#0f172a;font-size:17px;font-weight:800}.overview-stat{fill:#334155;font-size:12px;font-weight:700}.overview-axis line{stroke:#64748b;stroke-width:1.4}.overview-axis text{fill:#475569;font-size:12px;font-weight:650}.overview-axis .axis-label{fill:#1e293b;font-size:14px;font-weight:750}</style><pattern id="report-phase-te" width="8" height="8" patternUnits="userSpaceOnUse"><rect width="8" height="8" fill="#4B5563"/></pattern><pattern id="report-phase-ex" width="8" height="8" patternUnits="userSpaceOnUse"><rect width="8" height="8" fill="#0072B2"/></pattern><pattern id="report-phase-in" width="8" height="8" patternUnits="userSpaceOnUse"><rect width="8" height="8" fill="#009E73"/></pattern><pattern id="report-phase-re" width="8" height="8" patternUnits="userSpaceOnUse"><rect width="8" height="8" fill="#D55E00"/></pattern></defs><text x="24" y="28" class="overview-title">Mean CoI Composition by Condition</text><text x="776" y="28" text-anchor="end" class="overview-stat">PERMANOVA ${escapeHtml(academicPValue(report.global_test.p_value))} · R² = ${academicNumber(report.global_test.effect_size, 2)}</text>${stackedRows}<g class="overview-axis"><line x1="165" x2="165" y1="68" y2="194"/><line x1="165" x2="173" y1="83" y2="83"/><line x1="165" x2="173" y1="131" y2="131"/><line x1="165" x2="173" y1="179" y2="179"/><text x="28" y="131" transform="rotate(-90 28 131)" text-anchor="middle" class="axis-label">Experimental Condition</text><line x1="175" x2="735" y1="232" y2="232"/>${axisTicks}<text x="455" y="276" text-anchor="middle" class="axis-label">Mean Code Composition (%)</text></g></svg></section>`
  const panels = phases.map((phase, index) => {
    const test = report.statistical_tests.find(item => item.metric === phase.key)
    const valuesByCondition = Object.fromEntries(conditionColumns.map(condition => [condition, report.observations.filter(item => item.condition === condition).map(item => item[phase.key])]))
  ...
  const publicationCaption = language === 'zh'
    ? '<strong>图 1.</strong> 四个 CoI 阶段在三种实验条件下的合并论文图。面板右上角的 p 值为三条件总体检验经 BH 多重比较校正后的结果；组间括号上的 p 值为事后两两比较校正后的结果。n.s. 表示未达到显著水平，显著结果以红色显示。'
    : '<strong>Figure 1.</strong> Combined publication figure for all four CoI phases across the three experimental conditions. Panel p values are BH-adjusted three-condition tests; bracketed p values are adjusted post-hoc pairwise comparisons. n.s. indicates a non-significant result, and significant results are highlighted in red.'
