...
import type { AdminWindowMetricsKeyword } from '../../types/admin'
import {
  listWindowMetricsKeywords,
  deleteWindowMetricsKeyword,
  batchDeleteWindowMetricsKeywords,
} from '../../api/admin/window-metrics-keywords'
import { formatDateTimeToCST } from '../../utils/datetime'
import { exportRowsToCsv } from '../../utils/csv'

const loading = ref(false)
const rows = ref<AdminWindowMetricsKeyword[]>([])
const page = ref(1)
const pageSize = ref(20)
const total = ref(0)
const tableRef = ref<{ clearSelection: () => void } | null>(null)
const selectedRows = ref<AdminWindowMetricsKeyword[]>([])
...
async function fetchData() {
  ...
  try {
    ...
    const res = await listWindowMetricsKeywords({
      page: page.value,
    ...
    ElMessage.error(e?.message || '加载窗口关键词失败')
  } finally {
    loading.value = false
  }
}
...
function handleSelectionChange(r: AdminWindowMetricsKeyword[]) { selectedRows.value = r }
...
function handleExportCsv() {
  ...
  const ts = Date.now()
  exportRowsToCsv<AdminWindowMetricsKeyword>({
    filename: `窗口关键词-选中导出-${ts}.csv`,
...
async function handleDelete(row: AdminWindowMetricsKeyword) {
  try {
    await ElMessageBox.confirm('确认删除该窗口关键词记录吗？', '删除确认', {
      type: 'warning',
  ...
  try {
    await deleteWindowMetricsKeyword(row.id)
...
async function handleBatchDelete() {
  ...
  try {
    ...
    const res = await batchDeleteWindowMetricsKeywords(ids)
