...
用法（在 backend/ 目录下）：
    python -m tests.test_ai_push_analysis
...
URL = f"{BASE}/api/admin/ai-push-analysis"
SESSION_PREFIX = f"sess_test_apa_{uuid.uuid4().hex[:8]}"
SEEDED_IDS: list[str] = []
...
def cleanup_seed_rows() -> None:
    if not SEEDED_IDS:
        return
    try:
        asyncio.run(exec_sql("DELETE FROM ai_push_analysis WHERE id = ANY(:ids)", {"ids": SEEDED_IDS}))
...
def seed_rows() -> None:
    rows = [
        ...
        },
        {
            "id": f"apa_test_{uuid.uuid4().hex[:8]}",
            "session_id": SESSION_PREFIX,
            "target_user_id": "u_apa_group_silence",
            "state_type": "group_silence",
            "window_start": "2025-01-01 10:05:00",
            "ai_needs_prompt": "true",
            "ai_anchor": "NULL",
...
data = check("filter: session_id 精确匹配", requests.get(f"{URL}/?session_id={SESSION_PREFIX}", headers=HEADERS))
if data:
    assert_items_all(data, lambda item: item["session_id"] == SESSION_PREFIX, "session_id 过滤错误")

data = check("filter: target_user_id 精确匹配",
             requests.get(f"{URL}/?target_user_id=u_apa_persist_failed", headers=HEADERS))
if data:
...
if data:
    assert_items_all(data, lambda item: item["state_type"] == "group_silence", "group_silence 过滤错误")

data = check("filter: ai_needs_prompt=true",
             requests.get(f"{URL}/?session_id={SESSION_PREFIX}&ai_needs_prompt=true", headers=HEADERS))
if data:
    assert_items_all(data, lambda item: item["ai_needs_prompt"] is True, "ai_needs_prompt=true 过滤错误")

data = check("filter: ai_needs_prompt=false",
             requests.get(f"{URL}/?session_id={SESSION_PREFIX}&ai_needs_prompt=false", headers=HEADERS))
if data:
    assert_items_all(data, lambda item: item["ai_needs_prompt"] is False, "ai_needs_prompt=false 过滤错误")

check("filter: ai_needs_prompt=invalid → 422",
      requests.get(f"{URL}/?ai_needs_prompt=maybe", headers=HEADERS), 422)

for reason in ["passed", "needs_prompt_false", "anchor_invalid", "content_empty", "persist_failed"]:
    data = check(f"filter: drop_reason={reason}",
                 requests.get(f"{URL}/?session_id={SESSION_PREFIX}&drop_reason={reason}", headers=HEADERS))
    if data:
        assert_items_all(data, lambda item, expected=reason: item["drop_reason"] == expected, f"{reason} 过滤错误")

data = check("filter: window_start_from=2025-01-01T10:02:00Z",
             requests.get(f"{URL}/?session_id={SESSION_PREFIX}&window_start_from=2025-01-01T10:02:00Z", headers=HEADERS))
if data:
    assert_items_all(data, lambda item: item["window_start"] >= "2025-01-01T10:02:00", "window_start_from 过滤错误")

data = check("filter: window_start_to=2025-01-01T10:03:00Z",
             requests.get(f"{URL}/?session_id={SESSION_PREFIX}&window_start_to=2025-01-01T10:03:00Z", headers=HEADERS))
if data:
    assert_items_all(data, lambda item: item["window_start"] <= "2025-01-01T10:03:00", "window_start_to 过滤错误")

check("filter: window_start_from 非法时间格式 → 422",
      requests.get(f"{URL}/?window_start_from=not-a-date", headers=HEADERS), 422)
check("filter: window_start_to 非法时间格式 → 422",
      requests.get(f"{URL}/?window_start_to=not-a-date", headers=HEADERS), 422)

data = check("filter: 多条件组合（ai_needs_prompt + drop_reason）",
             requests.get(
                 f"{URL}/?session_id={SESSION_PREFIX}&ai_needs_prompt=true&drop_reason=persist_failed",
                 headers=HEADERS,
             ))
if data:
    assert_items_all(
        data,
        lambda item: item["ai_needs_prompt"] is True and item["drop_reason"] == "persist_failed",
        "组合过滤错误",
    )

data = check("filter: 多条件组合（state_type + window_start 范围）",
             requests.get(
                 f"{URL}/?session_id={SESSION_PREFIX}&state_type=group_silence"
                 "&window_start_from=2025-01-01T10:05:00Z&window_start_to=2025-01-01T10:05:00Z",
                 headers=HEADERS,
             ))
if data:
    assert_items_all(data, lambda item: item["state_type"] == "group_silence", "group_silence 范围过滤错误")
...
check("再次 DELETE 同一 id → 404", requests.delete(f"{URL}/{delete_id}", headers=HEADERS), 404)
...
r_bd = check("batch-delete 100 条不存在的 id（边界，deleted=0）",
...
check("target_user_id 含特殊字符",
      requests.get(f"{URL}/?target_user_id=user%27%3BDROP%20TABLE", headers=HEADERS))
check("state_type 含空格",
      requests.get(f"{URL}/?state_type=low+participation", headers=HEADERS))
check("drop_reason 非法值（任意字符串，后端当成无匹配）",
      requests.get(f"{URL}/?drop_reason=unknown_reason", headers=HEADERS))
